...
範本中必要欄位的欄名會以紅色系底色標示，`填表說明` 工作表也會列出每個欄位的定義、必填狀態、範例與備註。必要工作表只有 `待排工單` 與 `產品機台產速`；`機台初始狀態`、`換模時間` 可依現場資料填寫。每張工單若要限制只能使用特定機台，請在 `待排工單` 的 `工單限定機台` 填入 `C4` 或 `C4,C5`；空白代表依產品機台產速表判斷。舊檔的 `允許機台` 欄名仍可相容讀取。
