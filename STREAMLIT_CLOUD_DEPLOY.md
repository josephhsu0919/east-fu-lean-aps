...
公開網址代表知道連結的人都可能使用。若尚未加入登入控管，請勿上傳真實客戶訂單、價格、產能或任何敏感資料。

目前系統的歷史排程會寫入 `data/schedule_history.json`。這個檔案已加入 `.gitignore`，不要把本機測試歷史推到 GitHub。若部署成公開網址，建議定期清空部署端歷史，或後續改成登入後每家公司獨立保存。
...
公開網址階段請把歷史排程視為暫存操作資料。正式營運前，建議加上登入與每家公司獨立資料保存。
